...
class SeniorVisitor implements Visitor {
  ...
  visitHauntedHouse(hauntedHouse: HauntedHouse): void {
    console.log(
      `Adulto en Casa del Terror: Precio con descuento de $${hauntedHouse.getPrice() * 0.85
      }`
    );
  }
  ...
  visitFerrisWheel(ferrisWheel: FerrisWheel): void {
    console.log(
      `Adulto en la Rueda de la Fortuna: Precio con descuento de $${ferrisWheel.getPrice() * 0.85
      }`
    );
  }
}
